...
// implementation for dummy block
static IMP dummyBlockImp = NULL;
...
- (void)respondsToSelector:(SEL)selector withKey:(id)inKey usingBlock:(id)block
{
	...
	@synchronized (self) {
        ...
        IMP imp = imp_implementationWithBlock(copiedBlock);
		...
		if (blockInfo && blockInfos) {
			...
			if (blockInfo == [blockInfos lastObject]) {
				...
				if (supermethod) {
                    ...
                    if (dummyBlockImp == NULL)
                        dummyBlockImp = imp_implementationWithBlock(kDummyBlock);
					class_replaceMethod([self class], selector, dummyBlockImp, objCTypes);
				}
			}
