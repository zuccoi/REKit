...
- (void)test_supermethodPointsToNil
{
	...
	RESetBlock([NSArray class], sel, NO, @"key", ^(id receiver) {
		// Check supermethod
		STAssertNil((id)[re_receiver supermethodOfInstanceMethod:re_selector key:re_key], @"");
...
- (void)test_supermethodPointsToOriginalMethod
{
	...
	RESetBlock([RETestObject class], sel, NO, @"key", ^(id receiver) {
		// Check supermethod
		STAssertEquals([re_receiver supermethodOfInstanceMethod:re_selector key:re_key], originalMethod, @"");
...
- (void)test_supermethodPointsToMethodOfSuperclass
{
	...
	RESetBlock([RESubTestObject class], sel, NO, @"key", ^(id receiver) {
		// Check supermethod
		STAssertEquals([re_receiver supermethodOfInstanceMethod:re_selector key:re_key], [RETestObject instanceMethodForSelector:sel], @"");
...
- (void)test_supermethodPointsToInstancesBlockOfSuperclass
{
	...
	RESetBlock([RETestObject class], sel, NO, nil, ^(id receiver) {
		IMP supermethod;
		if ((supermethod = [re_receiver supermethodOfInstanceMethod:re_selector key:re_key])) {
...
- (void)test_supermethod__order
{
	...
	RESetBlock(testObj, sel, NO, nil, ^(id receiver) {
		// supermethod
		IMP supermethod;
		supermethod = [re_receiver supermethodOfInstanceMethod:re_selector key:re_key];
		if (supermethod) {
...
- (void)test_RE_IMP__void
{
	...
	RESetBlock([NSObject class], sel, NO, nil, ^(id receiver) {
		(RE_IMP(void)[re_receiver supermethodOfInstanceMethod:re_selector key:re_key])(receiver, sel);
	});
...
- (void)test_RE_IMP__id
{
	...
	RESetBlock([NSObject class], sel, NO, nil, ^(id receiver) {
		NSString *res;
		res = (RE_IMP(id)[re_receiver supermethodOfInstanceMethod:re_selector key:re_key])(receiver, sel);
		return res;
	});
...
- (void)test_RE_IMP__scalar
{
	...
	RESetBlock([NSObject class], sel, NO, nil, ^(id receiver) {
		NSInteger i;
		i = (RE_IMP(NSInteger)[re_receiver supermethodOfInstanceMethod:re_selector key:re_key])(receiver, sel);
		return i + 1;
	});
...
- (void)test_hasDynamicBlockAddedAfterKVO
{
	...
	// Add block
	RESetBlock([NSObject class], _cmd, NO, RE_FUNC, ^(Class class) {
	});
	
	// Check
	STAssertTrue([NSObject hasBlockForInstanceMethod:_cmd key:RE_FUNC], @"");
...
- (void)test_hasOverrideBlockAddedAfterKVO
{
	...
	// Add block
	RESetBlock([RETestObject class], @selector(log), NO, RE_FUNC, ^(Class class) {
	});
	
	// Check
	STAssertTrue([RETestObject hasBlockForInstanceMethod:@selector(log) key:RE_FUNC], @"");
